...
void get_block(uint8_t* block,uint8_t* Scanline,int pos){
    int index = 0;
    for(int i = pos; i<pos+bytes_pp;i++){
        block[index] = Scanline[i];
        index++;
    }
}

void Add_blocks(uint8_t* last,uint8_t* curr,uint8_t* Scanline,int pos){
...
void Undo_Sub(uint8_t* Scanline, int ScanLineLength){
    ...
    for(int i = 0 ; i < ScanLineLength-bytes_pp; i+=bytes_pp){
                if(first){
                get_block(lastblock,ScanLine,i);
                get_block(lastblock,ScanLine,i);
                first = 0;
                }
                else{
                get_block(lastblock,ScanLine,i-bytes_pp);
                get_block(currblock,ScanLine,i);
...
void Undo_Avg(){
   ...
   // uint8_t* curr_block=malloc(bytes_pp);    
    for(int i = 0 ; i < ScanLineLength-bytes_pp; i+=bytes_pp){
       if(i==0){
                get_block(lastblock,ScanLine,i);
                get_block(currblock,previous_ScanLine,i);
...
void Undo_Paeth(){
   ...
   for(int i=0; i< ScanLineLength-bytes_pp;i+=bytes_pp){
         if(i==0){
                get_block(lastblock,ScanLine,i);
                get_block(currblock,ScanLine,i);
                get_block(last_scan_curr,previous_ScanLine,i);
                get_block(last_scan_curr,previous_ScanLine,i);
                for(int j = i; j<bytes_pp+i;j++){
                   a = 0;
                ...
                get_block(currblock,ScanLine,i);
                get_block(last_scan_curr,previous_ScanLine,i);
                get_block(last_scan_last,previous_ScanLine,i-bytes_pp);
                for(int j = i; j<bytes_pp+i;j++){
                   a = lastblock[j-i];
                   b = last_scan_curr[j-i];
...
void Undo_Filters(){
    ...
    currblock = malloc(bytes_pp);
    lastblock = malloc(bytes_pp);
